...
export default function ContractPanel({
  ...
  const isActive = (tab) => activeTab === tab || (!canShowTabs && tab === "info");
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 z-30">
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl border overflow-hidden flex flex-col max-h-[90vh]">
        <header className="px-6 py-5 border-b flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold">{title}</h2>
            {editing.id && (
              <p className="text-sm text-gray-500">ID: {editing.id.slice(0, 8)}…</p>
            )}
          </div>
            ...
            >
              ...
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <label className="flex flex-col gap-1">
                    <span className="text-sm text-gray-600">Precio inicial (ARS)</span>
                    <input
                      type="number"
                      min={0}
                      required
              ...
              {editing.id && (
                <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label className="flex flex-col gap-1">
                      <span className="text-sm text-gray-600">Precio vigente (mes actual)</span>
                      <input
                        readOnly
                        value={
                          currentMonthlyTotal != null ? fmtMoney(currentMonthlyTotal) : "-"
                        }
                        className="px-3 py-2 border rounded-xl bg-white text-gray-700 font-semibold"
                      />
                    </label>
                    <label className="flex flex-col gap-1">
                      <span className="text-sm text-gray-600">Último precio calculado</span>
                      <input
                        readOnly
                        value={lastPriceValue != null ? fmtMoney(lastPriceValue) : "-"}
                        className="px-3 py-2 border rounded-xl bg-white text-gray-700"
                      />
                    </label>
                    <label className="flex flex-col gap-1">
                      <span className="text-sm text-gray-600">Precio vigente desde</span>
                      <input
                        readOnly
                        value={lastPriceSince ? fmtDateAR(lastPriceSince) : "-"}
                        className="px-3 py-2 border rounded-xl bg-white text-gray-700"
                      />
                    </label>
                  </div>
